...
test.describe
	.serial("Environment Variable Tests", () => {
		...
		test.describe("Custom Title and Description", () => {
			test.beforeAll(async () => {
				...
				serverProcess = await startBuild(projectName, projectPort.toString(), {
					...
					MAINTENANCE_DESCRIPTION:
						"This description comes from environment variables.",
				});
			});
			...
			test.afterAll(async () => {
				...
				console.log(
					chalk.bgYellowBright.black("Title and description test complete!"),
				);
			});

			test("Shows maintenance page with customized title and description from env vars", async ({
				page,
			}) => {
				...
				await expect(page.getByText("Custom Environment Title")).toBeVisible({
					timeout: 10000,
				});
				await expect(
					page.getByText("This description comes from environment variables."),
				).toBeVisible();

				console.log(
					chalk.bgGreenBright.white("Title & description env var test passed!"),
				);
			});
		});
		...
		test.describe("Custom Override Parameter", () => {
			...
			test.afterAll(async () => {
				...
				console.log(
					chalk.bgYellowBright.black("Override parameter test complete!"),
				);
			});
			...
			test("Custom override parameter works", async ({ page }) => {
				...
				await page.goto(
					`http://localhost:${projectPort.toString()}/?custom-override`,
				);
				await page.waitForLoadState("networkidle");
				...
				console.log(
					chalk.bgGreenBright.white("Custom override parameter test passed!"),
				);
			});
		});
		...
		test.describe("Disabling Maintenance Mode", () => {
			...
			test("Maintenance mode is disabled via env var", async ({ page }) => {
				...
				console.log(
					chalk.bgGreenBright.white("Disabled mode env var test passed!"),
				);
			});
		});
		...
		test.describe("Contact Information", () => {
			...
			test("Shows contact info from env vars", async ({ page }) => {
				...
				console.log(
					chalk.bgGreenBright.white("Contact info env var test passed!"),
				);
			});
		});
		...
		test.describe("Cookie Max Age", () => {
			...
			test.afterAll(async () => {
				...
				console.log(
					chalk.bgYellowBright.black("Cookie max age test complete!"),
				);
			});

			test("Sets cookie with custom max age from env var", async ({
				page,
				context,
			}) => {
				// First access with override to set the cookie
				await page.goto(
					`http://localhost:${projectPort.toString()}/?override-test`,
				);
				await page.waitForLoadState("networkidle");
				...
				const maintenanceCookie = cookies.find(
					(c) => c.name === "astro_maintenance_override",
				);
				...
				if (maintenanceCookie) {
					...
					const maxAgeInSeconds = Math.round(
						maintenanceCookie.expires! - Date.now() / 1000,
					);
				...
				console.log(
					chalk.bgGreenBright.white("Cookie max age env var test passed!"),
				);
			});
		});
	});
